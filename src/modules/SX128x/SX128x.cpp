...
SX128x::SX128x(Module* mod)
    : PhysicalLayer(RADIOLIB_SX128X_FREQUENCY_STEP_SIZE,
                    RADIOLIB_SX128X_MAX_PACKET_LENGTH) {
  this->mod = mod;
}

int16_t SX128x::begin(float freq, float bw, uint8_t sf, uint8_t cr,
                      uint8_t syncWord, int8_t pwr, uint16_t preambleLength) {
  ...
  this->codingRateLoRa = RADIOLIB_SX128X_LORA_CR_4_5;
...
int16_t SX128x::beginGFSK(float freq, uint16_t br, float freqDev, int8_t pwr,
...
int16_t SX128x::beginBLE(float freq, uint16_t br, float freqDev, int8_t pwr,
                         uint8_t dataShaping) {
  // set module properties
  this->mod->init();
  this->mod->hal->pinMode(this->mod->getIrq(), this->mod->hal->GpioModeInput);
...
int16_t SX128x::beginFLRC(float freq, uint16_t br, uint8_t cr, int8_t pwr,
                          uint16_t preambleLength, uint8_t dataShaping) {
  // set module properties
  this->mod->init();
  this->mod->hal->pinMode(this->mod->getIrq(), this->mod->hal->GpioModeInput);
  this->mod->hal->pinMode(this->mod->getGpio(), this->mod->hal->GpioModeInput);
...
int16_t SX128x::transmitDirect(uint32_t frf) {
  ...
  // start transmitting
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_TX_CONTINUOUS_WAVE,
                                    NULL, 0));
}
...
int16_t SX128x::scanChannel() {
  ...
  state = setDioIrqParams(
      RADIOLIB_SX128X_IRQ_CAD_DETECTED | RADIOLIB_SX128X_IRQ_CAD_DONE,
      RADIOLIB_SX128X_IRQ_CAD_DETECTED | RADIOLIB_SX128X_IRQ_CAD_DONE);
  RADIOLIB_ASSERT(state);
...
int16_t SX128x::sleep(bool retainConfig) {
  ...
  uint8_t sleepConfig = RADIOLIB_SX128X_SLEEP_DATA_BUFFER_RETAIN |
                        RADIOLIB_SX128X_SLEEP_DATA_RAM_RETAIN;
  if (!retainConfig) {
    sleepConfig = RADIOLIB_SX128X_SLEEP_DATA_BUFFER_FLUSH |
                  RADIOLIB_SX128X_SLEEP_DATA_RAM_FLUSH;
  }
  int16_t state = this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_SLEEP,
                                            &sleepConfig, 1, false, false);
...
void SX128x::clearPacketReceivedAction() { this->clearDio1Action(); }

void SX128x::setPacketSentAction(void (*func)(void)) {
  this->setDio1Action(func);
}

void SX128x::clearPacketSentAction() { this->clearDio1Action(); }
...
int16_t SX128x::startTransmit(uint8_t* data, size_t len, uint8_t addr) {
  ...
  if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
    state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, len,
                                this->crcLoRa, this->invertIQEnabled);
  } else if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) ||
             (modem == RADIOLIB_SX128X_PACKET_TYPE_FLRC)) {
    state = setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen,
                                this->syncWordMatch, this->crcGFSK,
                                this->whitening, len);
  } else if (modem == RADIOLIB_SX128X_PACKET_TYPE_BLE) {
    state = setPacketParamsBLE(this->connectionState, this->crcBLE,
                               this->bleTestPayload, this->whitening);
  } else {
    return (RADIOLIB_ERR_WRONG_MODEM);
  }
  RADIOLIB_ASSERT(state);
  ...
  // set DIO mapping
  state = setDioIrqParams(
      RADIOLIB_SX128X_IRQ_TX_DONE | RADIOLIB_SX128X_IRQ_RX_TX_TIMEOUT,
...
int16_t SX128x::startReceive() {
  return (this->startReceive(RADIOLIB_SX128X_RX_TIMEOUT_INF,
                             RADIOLIB_SX128X_IRQ_RX_DEFAULT,
                             RADIOLIB_SX128X_IRQ_RX_DONE, 0));
}

int16_t SX128x::startReceive(uint16_t timeout, uint16_t irqFlags,
                             uint16_t irqMask, size_t len) {
  (void)len;
...
int16_t SX128x::readData(uint8_t* data, size_t len) {
  ...
  if ((irq & RADIOLIB_SX128X_IRQ_CRC_ERROR) ||
      (irq & RADIOLIB_SX128X_IRQ_HEADER_ERROR)) {
...
int16_t SX128x::setFrequency(float freq) {
  ...
  // calculate raw value
  uint32_t frf = (freq * (uint32_t(1) << RADIOLIB_SX128X_DIV_EXPONENT)) /
                 RADIOLIB_SX128X_CRYSTAL_FREQ;
...
int16_t SX128x::setSpreadingFactor(uint8_t sf) {
  ...
  int16_t state = setModulationParams(this->spreadingFactor, this->bandwidth,
                                      this->codingRateLoRa);
  RADIOLIB_ASSERT(state);
...
int16_t SX128x::setCodingRate(uint8_t cr, bool longInterleaving) {
  ...
  if ((modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) ||
    ...
    return (setModulationParams(this->bitRate, this->codingRateFLRC,
                                this->shaping));
  }
...
int16_t SX128x::setPreambleLength(uint32_t preambleLength) {
  ...
  if ((modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) ||
      (modem == RADIOLIB_SX128X_PACKET_TYPE_RANGING)) {
    // LoRa or ranging
    RADIOLIB_CHECK_RANGE(preambleLength, 2, 491520,
    ...
    return (setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType,
                                this->payloadLen, this->crcLoRa,
                                this->invertIQEnabled));
             ...
             (modem == RADIOLIB_SX128X_PACKET_TYPE_FLRC)) {
    // GFSK or FLRC
    RADIOLIB_CHECK_RANGE(preambleLength, 4, 32,
                         RADIOLIB_ERR_INVALID_PREAMBLE_LENGTH);
    ...
    return (setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen,
                                this->syncWordMatch, this->crcGFSK,
                                this->whitening));
  }
...
int16_t SX128x::setBitRate(float br) {
  ...
  if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) ||
      (modem == RADIOLIB_SX128X_PACKET_TYPE_BLE)) {
    if ((uint16_t)br == 125) {
...
int16_t SX128x::setFrequencyDeviation(float freqDev) {
  ...
  RADIOLIB_CHECK_RANGE(newFreqDev, 62.5, 1000.0,
  ...
  // update modulation parameters
  uint8_t modIndex =
      (uint8_t)((8.0 * (newFreqDev / (float)this->bitRateKbps)) - 1.0);
...
int16_t SX128x::setSyncWord(uint8_t* syncWord, uint8_t len) {
  ...
  // update sync word
  int16_t state = SX128x::writeRegister(RADIOLIB_SX128X_REG_SYNC_WORD_1_BYTE_4,
                                        syncWordBuff, 5);
  RADIOLIB_ASSERT(state);
...
int16_t SX128x::setCRC(uint8_t len, uint32_t initial, uint16_t polynomial) {
  ...
  if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) ||
    ...
    uint8_t data[] = {(uint8_t)((initial >> 8) & 0xFF),
    ...
    // set initial CRC value
    uint8_t data[] = {(uint8_t)((initial >> 16) & 0xFF),
                      (uint8_t)((initial >> 8) & 0xFF),
                      (uint8_t)(initial & 0xFF)};
    state = writeRegister(RADIOLIB_SX128X_REG_BLE_CRC_INITIAL_MSB, data, 3);
    return (state);

  } else if ((modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) ||
             (modem == RADIOLIB_SX128X_PACKET_TYPE_RANGING)) {
    // update packet parameters
    if (len == 0) {
...
int16_t SX128x::setWhitening(bool enabled) {
  ...
  return (setPacketParamsBLE(this->connectionState, this->crcBLE,
                             this->bleTestPayload, this->whitening));
}
...
int16_t SX128x::setAccessAddress(uint32_t addr) {
  ...
  // set the address
  uint8_t addrBuff[] = {(uint8_t)((addr >> 24) & 0xFF),
                        (uint8_t)((addr >> 16) & 0xFF),
                        (uint8_t)((addr >> 8) & 0xFF), (uint8_t)(addr & 0xFF)};
  return (SX128x::writeRegister(RADIOLIB_SX128X_REG_ACCESS_ADDRESS_BYTE_3,
                                addrBuff, 4));
}
...
int16_t SX128x::setGainControl(uint8_t gain) {
  ...
  int16_t state = readRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_CONTROL_ENABLE_2,
                               &ManualGainSetting, 1);
  RADIOLIB_ASSERT(state);
  uint8_t LNAGainValue = 0;
  state =
      readRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_SETTING, &LNAGainValue, 1);
  RADIOLIB_ASSERT(state);
  uint8_t LNAGainControl = 0;
  state = readRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_CONTROL_ENABLE_1,
                       &LNAGainControl, 1);
  RADIOLIB_ASSERT(state);
  ...
  if (gain > 0 && gain < 14) {
    ...
    LNAGainValue |=
        gain;  // Set bits 0, 1, 2 and 3 to Manual Gain Setting (1-13)
    LNAGainControl |= 0x80;  // Set bit 7 to 1 (Enable Manual Gain Control)
  } else {
    // Set automatic gain if 0 or out of range
    ManualGainSetting |=
        0x01;              // Set bit 0 to 1 (Enable Automatic Gain Control)
    LNAGainValue &= 0xF0;  // Bits 0, 1, 2 and 3 to 0
    LNAGainValue |=
        0x0A;  // Set bits 0, 1, 2 and 3 to Manual Gain Setting (1-13)
    LNAGainControl &= ~0x80;  // Set bit 7 to 0 (Enable Automatic Gain Control)
  }

  // update all values
  state = writeRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_CONTROL_ENABLE_2,
                        &ManualGainSetting, 1);
  RADIOLIB_ASSERT(state);
  state =
      writeRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_SETTING, &LNAGainValue, 1);
  RADIOLIB_ASSERT(state);
  state = writeRegister(RADIOLIB_SX128X_REG_MANUAL_GAIN_CONTROL_ENABLE_1,
                        &LNAGainControl, 1);
  return (state);
}
...
float SX128x::getRSSI() {
  ...
  this->mod->SPIreadStream(RADIOLIB_SX128X_CMD_GET_PACKET_STATUS, packetStatus,
                           5);
...
size_t SX128x::getPacketLength(bool update) {
  ...
  this->mod->SPIreadStream(RADIOLIB_SX128X_CMD_GET_RX_BUFFER_STATUS,
                           rxBufStatus, 2);
...
uint32_t SX128x::getTimeOnAir(size_t len) {
  ...
  if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
    ...
    if (this->codingRateLoRa <= RADIOLIB_SX128X_LORA_CR_4_8) {
      ...
      uint32_t N_symbolPreamble =
          (this->preambleLengthLoRa & 0x0F) *
          (uint32_t(1) << ((this->preambleLengthLoRa & 0xF0) >> 4));

      // calculate the number of symbols
      N_symbol = (float)N_symbolPreamble + coeff1 + 8.0 +
                 ceil(RADIOLIB_MAX((int16_t)(8 * len + N_bitCRC - coeff2 +
                                             N_symbolHeader),
                                   (int16_t)0) /
                      (float)coeff3) *
                     (float)(this->codingRateLoRa + 4);
...
Module* SX128x::getMod() { return (this->mod); }
...
int16_t SX128x::setTx(uint16_t periodBaseCount, uint8_t periodBase) {
  uint8_t data[] = {periodBase, (uint8_t)((periodBaseCount >> 8) & 0xFF),
...
int16_t SX128x::setRfFrequency(uint32_t frf) {
  uint8_t data[] = {(uint8_t)((frf >> 16) & 0xFF), (uint8_t)((frf >> 8) & 0xFF),
                    (uint8_t)(frf & 0xFF)};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_RF_FREQUENCY, data, 3));
}

int16_t SX128x::setTxParams(uint8_t pwr, uint8_t rampTime) {
  uint8_t data[] = {pwr, rampTime};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_TX_PARAMS, data, 2));
}

int16_t SX128x::setBufferBaseAddress(uint8_t txBaseAddress,
                                     uint8_t rxBaseAddress) {
  uint8_t data[] = {txBaseAddress, rxBaseAddress};
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_BUFFER_BASE_ADDRESS,
                                    data, 2));
}

int16_t SX128x::setModulationParams(uint8_t modParam1, uint8_t modParam2,
                                    uint8_t modParam3) {
  uint8_t data[] = {modParam1, modParam2, modParam3};
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_MODULATION_PARAMS,
                                    data, 3));
}

int16_t SX128x::setPacketParamsGFSK(uint8_t preambleLen, uint8_t syncLen,
                                    uint8_t syncMatch, uint8_t crcLen,
                                    uint8_t whiten, uint8_t payLen,
                                    uint8_t hdrType) {
  uint8_t data[] = {preambleLen, syncLen, syncMatch, hdrType,
                    payLen,      crcLen,  whiten};
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_PACKET_PARAMS, data,
                                    7));
}

int16_t SX128x::setPacketParamsBLE(uint8_t connState, uint8_t crcLen,
                                   uint8_t bleTest, uint8_t whiten) {
  uint8_t data[] = {connState, crcLen, bleTest, whiten, 0x00, 0x00, 0x00};
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_PACKET_PARAMS, data,
                                    7));
}

int16_t SX128x::setPacketParamsLoRa(uint8_t preambleLen, uint8_t hdrType,
                                    uint8_t payLen, uint8_t crc,
                                    uint8_t invIQ) {
...
int16_t SX128x::setDioIrqParams(uint16_t irqMask, uint16_t dio1Mask,
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_DIO_IRQ_PARAMS,
                                    data, 8));
}
...
int16_t SX128x::clearIrqStatus(uint16_t clearIrqParams) {
  ...
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_CLEAR_IRQ_STATUS, data, 2));
}

int16_t SX128x::setRangingRole(uint8_t role) {
  uint8_t data[] = {role};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_RANGING_ROLE, data, 1));
}

int16_t SX128x::setPacketType(uint8_t type) {
  uint8_t data[] = {type};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_PACKET_TYPE, data, 1));
}
...
int16_t SX128x::config(uint8_t modem) {
  ...
  state =
      this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_CAD_PARAMS, data, 1);
  RADIOLIB_ASSERT(state);

  // set regulator mode to DC-DC
  data[0] = RADIOLIB_SX128X_REGULATOR_DC_DC;
  state = this->mod->SPIwriteStream(RADIOLIB_SX128X_CMD_SET_REGULATOR_MODE,
                                    data, 1);
  RADIOLIB_ASSERT(state);
