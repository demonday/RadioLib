...
Module::Module(RadioLibHal* hal, uint32_t cs, uint32_t irq, uint32_t rst,
               uint32_t gpio)
    : csPin(cs), irqPin(irq), rstPin(rst), gpioPin(gpio) {
  this->hal = hal;
}

Module::Module(const Module& mod) { *this = mod; }
...
void Module::init() {
  ...
  RADIOLIB_DEBUG_BASIC_PRINTLN("Version:  %d.%d.%d.%d", RADIOLIB_VERSION_MAJOR,
  ...
  RADIOLIB_DEBUG_BASIC_PRINTLN("  csPin: %d", csPin);
  RADIOLIB_DEBUG_BASIC_PRINTLN("  irqPin: %d", this->irqPin);
  RADIOLIB_DEBUG_BASIC_PRINTLN("  rstPin: %d", this->rstPin);
  RADIOLIB_DEBUG_BASIC_PRINTLN("  gpio: %d\n", this->gpioPin);
}
...
int16_t Module::SPIsetRegValue(uint16_t reg, uint8_t value, uint8_t msb,
                               uint8_t lsb, uint8_t checkInterval,
                               uint8_t checkMask) {
  if ((msb > 7) || (lsb > 7) || (lsb > msb)) {
    return (RADIOLIB_ERR_INVALID_BIT_RANGE);
  }
...
void Module::SPIreadRegisterBurst(uint16_t reg, size_t numBytes,
                                  uint8_t* inBytes) {
  if (!SPIstreamType) {
    SPItransfer(SPIreadCommand, reg, NULL, inBytes, numBytes);
  } else {
    uint8_t cmd[] = {SPIreadCommand, (uint8_t)((reg >> 8) & 0xFF),
                     (uint8_t)(reg & 0xFF)};
    SPItransferStream(cmd, 3, false, NULL, inBytes, numBytes, true,
                      RADIOLIB_MODULE_SPI_TIMEOUT);
  }
}
...
uint8_t Module::SPIreadRegister(uint16_t reg) {
  ...
  if (!SPIstreamType) {
    ...
    uint8_t cmd[] = {SPIreadCommand, (uint8_t)((reg >> 8) & 0xFF),
                     (uint8_t)(reg & 0xFF)};
    SPItransferStream(cmd, 3, false, NULL, &resp, 1, true,
                      RADIOLIB_MODULE_SPI_TIMEOUT);
  }
  return (resp);
}

void Module::SPIwriteRegisterBurst(uint16_t reg, uint8_t* data,
                                   size_t numBytes) {
  if (!SPIstreamType) {
    SPItransfer(SPIwriteCommand, reg, data, NULL, numBytes);
  } else {
    uint8_t cmd[] = {SPIwriteCommand, (uint8_t)((reg >> 8) & 0xFF),
                     (uint8_t)(reg & 0xFF)};
    SPItransferStream(cmd, 3, true, data, NULL, numBytes, true,
                      RADIOLIB_MODULE_SPI_TIMEOUT);
  }
}
...
void Module::SPIwriteRegister(uint16_t reg, uint8_t data) {
  if (!SPIstreamType) {
    ...
    uint8_t cmd[] = {SPIwriteCommand, (uint8_t)((reg >> 8) & 0xFF),
                     (uint8_t)(reg & 0xFF)};
    SPItransferStream(cmd, 3, true, &data, NULL, 1, true,
                      RADIOLIB_MODULE_SPI_TIMEOUT);
  }
}

void Module::SPItransfer(uint8_t cmd, uint16_t reg, uint8_t* dataOut,
                         uint8_t* dataIn, size_t numBytes) {
  // prepare the buffers
...
int16_t Module::SPIreadStream(uint8_t cmd, uint8_t* data, size_t numBytes,
                              bool waitForGpio, bool verify) {
  return (this->SPIreadStream(&cmd, 1, data, numBytes, waitForGpio, verify));
}

int16_t Module::SPIreadStream(uint8_t* cmd, uint8_t cmdLen, uint8_t* data,
                              size_t numBytes, bool waitForGpio, bool verify) {
  // send the command
  int16_t state =
      this->SPItransferStream(cmd, cmdLen, false, NULL, data, numBytes,
                              waitForGpio, RADIOLIB_MODULE_SPI_TIMEOUT);
...
int16_t Module::SPIwriteStream(uint8_t cmd, uint8_t* data, size_t numBytes,
                               bool waitForGpio, bool verify) {
  return (this->SPIwriteStream(&cmd, 1, data, numBytes, waitForGpio, verify));
}

int16_t Module::SPIwriteStream(uint8_t* cmd, uint8_t cmdLen, uint8_t* data,
                               size_t numBytes, bool waitForGpio, bool verify) {
  // send the command
  int16_t state =
      this->SPItransferStream(cmd, cmdLen, true, data, NULL, numBytes,
                              waitForGpio, RADIOLIB_MODULE_SPI_TIMEOUT);
  RADIOLIB_ASSERT(state);
...
int16_t Module::SPIcheckStream() {
  ...
  state = this->SPItransferStream(&cmd, 1, false, NULL, &spiStatus, 0, true,
                                  RADIOLIB_MODULE_SPI_TIMEOUT);
  RADIOLIB_ASSERT(state);
...
int16_t Module::SPItransferStream(uint8_t* cmd, uint8_t cmdLen, bool write,
                                  uint8_t* dataOut, uint8_t* dataIn,
  ...
  if (waitForGpio) {
    if (this->gpioPin == RADIOLIB_NC) {
      ...
      while (this->hal->digitalRead(this->gpioPin)) {
        ...
        if (this->hal->millis() - start >= timeout) {
          RADIOLIB_DEBUG_BASIC_PRINTLN(
              "GPIO post-transfer timeout, is it connected?");
#if !RADIOLIB_STATIC_ONLY
          delete[] buffOut;
          delete[] buffIn;
#endif
  ...
  if ((this->SPIparseStatusCb != nullptr) && (numBytes > 0)) {
    ...
    RADIOLIB_DEBUG_SPI_PRINT("STATUS In: 0x%02x, Result: %d\n", buffIn[cmdLen],
                             state);
  }
...
#if RADIOLIB_DEBUG
void Module::hexdump(const char* level, uint8_t* data, size_t len,
                     uint32_t offset, uint8_t width, bool be) {
  size_t rem_len = len;
...
void Module::setRfSwitchTable(const uint32_t (&pins)[3],
...
void Module::setRfSwitchState(uint8_t mode) {
  ...
  for (size_t i = 0; i < RFSWITCH_MAX_PINS; i++) {
    ...
    if (pin != RADIOLIB_NC) this->hal->digitalWrite(pin, *value);
    ++value;
  }
}
